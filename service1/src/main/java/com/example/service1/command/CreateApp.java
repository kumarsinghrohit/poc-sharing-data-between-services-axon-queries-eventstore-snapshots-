...
public final class CreateApp {
    ...
    public static class CreateAppBuilder {
        private CreateAppBuilder() {
        }
    }

    public static CreateAppBuilder builder(AppId id, AppName name, AppType type, AppVersionId versionId,
            AppVersionNumber versionNumber, AppBinaryName binaryName, DeveloperId developerId, LocalDate createdOn) {
        return new CreateAppBuilder().id(id).name(name).type(type).versionId(versionId).versionNumber(versionNumber)
